...
Every field was resolved when the answer was given and stored on the message, so a
document deleted since is shown as it was, marked as no longer in the library.

Sources are always visible rather than behind a toggle.
"""
...
# What the backend calls a chunk's type, and what a reader should see: a table or a
# figure caption is read differently from a sentence.
...
def render(citations: list[dict], documents: list[dict], key_prefix: str) -> None:
    ...
    for citation in citations:
        ...
        with st.expander(heading):
            ...
            # Offered even for the page already beside the answer: otherwise
            # closing the panel left no way back to it.
            st.button(
                f"Show page {page}",
...
def as_rows(snippet: str) -> list[list[str]] | None:
    ...
    Handed to `st.markdown` inside a blockquote, a table arrives as one long line of
    pipes and dashes - unreadable, in the case where a reader most wants to check.

    A short last row is padded rather than dropped: those cells are still what the
    answer was read from.
    """
    lines = [line.strip() for line in snippet.splitlines() if line.strip()]
    lines = [line for line in lines if line != TRUNCATED_MARK]

    # Answers stored before the backend kept a table's line breaks are one long
    # line, and a stored citation is never resolved again. `| |` is where one row's
    # last cell meets the next row's first.
    if len(lines) == 1:
...
def _render_snippet(snippet: str, element_type: str) -> None:
    ...
    if element_type == "table":
        ...
        if rows:
            # Markdown rather than a dataframe: a dataframe clips each cell at the
            # column width, which is the half-sentence problem again. Markdown wraps.
...
def cited_page(conversation: dict | None) -> dict | None:
    ...
    Chosen by the answer rather than by a click, because checking the claim against
    its source is the point of the product.

    Only the newest answer counts. A greeting or a refusal cites nothing, and showing
    an older page beside one would suggest a source the answer never had.
    """
    if not conversation:
        return None
    ...
    # The reader's choice wins, but only over the answer it was made about: a new
    # answer cites something else.
...
def page_panel(citation: dict) -> None:
    ...
    A dialog covered the answer it was meant to support, so checking a citation meant
    trusting memory for what the answer said.

    It stays open while reading continues: another source swaps the page, and only
    Close dismisses it.
    """
    ...
    try:
        ...
        # Usually the original file is gone. The passage is still shown: it is what
        # the answer used, and better than an empty frame.
