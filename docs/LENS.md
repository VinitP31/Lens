...
Two extra PDFs are kept **outside** the library as stress tests: one scanned with no text layer, one with two columns. These check that the pipeline degrades gracefully. They sit in `samples/` under a `stress_` prefix and are never indexed — both the evaluation script and the corpus audit skip that prefix explicitly. They are generated by `scripts/make_stress_pdfs.py` so their content is known, and run through `scripts/profile_pdf.py --ocr`.

**Results.**

| | Pages | Characters a page | Outcome |
|---|---|---|---|
| `stress_scanned.pdf` | 2 | 0 → 275 with OCR | 6 passages, 100% coordinate coverage, citations highlight correctly |
| `stress_two_column.pdf` | 1 | 1058 | 4 passages, columns read one after the other rather than interleaved |

The two-column case is the one that would fail silently: read row by row, the two columns interleave into nonsense that is still fluent enough to answer from. It reads correctly.
...
**A subprocess, not a multiprocessing child.** This began as `multiprocessing` with the "spawn" method, which is the usual answer and was wrong here. A child started that way inherits the parent's open file descriptors, and the parent holds a live gRPC connection to the vector store. Measured: the child died with SIGTRAP and an empty stderr — no traceback, no message, just a document that never indexed. The worker is now launched as a plain subprocess with its descriptors closed, so it shares nothing with the parent.

The underlying cause is worth recording because it is not ours: **Milvus Lite leaks roughly ten file descriptors per connection, even after `close()`**. Six connections reached 66 open descriptors; a test suite making a dozen reached several hundred, and starting a child from that state is what killed it. The backend opens one connection for its lifetime and never approaches it, which is why uploads there always succeeded — the failure only reproduced under the test suite.

A worker killed by a signal without writing a result is therefore **retried once**. A worker that failed on its own — a corrupt PDF, no text — writes the reason and is not retried, because it would fail identically.
...
**Engine:** RapidOCR, which needs no system package. Tesseract would make a clean clone depend on a working `brew` or `apt` install before anything ran.

**Measured** on the scanned stress PDF, a page that is a picture of text with no text layer at all:

| | Before OCR | After |
|---|---|---|
| Characters a page | 0 | 275 |
| Passages recovered | 0 | 6 |
| Coordinate coverage | — | 100% |

That last row is the one that matters: a citation into a scanned document highlights the right lines on the page image, so an OCR'd answer is as checkable as any other. Verified by eye, not inferred.

A scan with nothing on it is rejected rather than indexed, naming the density found and the density needed.
...
| `OCR_LANGUAGES` | English | Matches the embedding model. Recognising text the rest of the system cannot search would cost time for nothing |
...
| `RENDER_DPI` | 150 | Readable on screen without a large file. A Letter page is about 1275×1650 |
| `HIGHLIGHT_FILL`, `HIGHLIGHT_OPACITY` | translucent amber | The words underneath must stay readable — the point is to show where the answer came from, not to cover it |
| `HIGHLIGHT_BORDER`, `HIGHLIGHT_BORDER_WIDTH`, `HIGHLIGHT_PADDING` | — | A box tight around one line reads as an underline rather than a highlight |
...
| The evaluation script cannot run while the backend is up | Milvus Lite allows one process to hold the store | Stop the server before measuring | A server build of Milvus, which accepts concurrent clients |
| Milvus Lite leaks file descriptors per connection | Around ten each, not released by `close()` | Harmless in the backend, which opens one connection for its lifetime. A process opening many eventually cannot start a subprocess | Reuse one connection, which the backend already does |
...
Universal OCR triples ingest time for nothing on normal PDFs. Averaging density document-wide avoids misflagging a good PDF with a few image pages. A cross-platform default means the repo runs on anyone's machine. *Rejected:* always-on OCR; per-page triggering; a mac-only engine as default. *Costs:* a config option, and a faster platform engine left unused by default. *Measured:* on a page that is only a picture of text, 0 characters a page became 275 across 6 passages with full coordinate coverage, so a citation into a scanned document still highlights the right lines. On the six real documents OCR did not fire and the chunk counts were unchanged, which is the result that matters — the slow path must stay out of the way.
...
Docling and Milvus Lite each bundle a copy of the OpenMP runtime, which refuses to initialise twice in one process and aborts when it does. It only triggers once the vector index is loaded, so an empty library ingests and a non-empty one crashes — invisible on the first run, certain afterwards. Docling is therefore imported only in a worker, launched as a plain subprocess with its descriptors closed and returning plain-typed chunks. *Rejected:* the environment variable that suppresses the abort, whose own documentation says it may silently produce incorrect results — worse than a crash, because a crash is visible; deduplicating the two library copies by hand, which no fresh install would reproduce; and `multiprocessing`, whose child inherits the parent's descriptors and was measured dying with SIGTRAP and an empty stderr when the parent held a live gRPC connection. A signal-killed worker is retried once; one that reported its own failure is not. *Costs:* the models reload per document, roughly 40s against 18s. *Buys:* per-document isolation, and the ingestion/query separation becomes structural rather than a convention.
